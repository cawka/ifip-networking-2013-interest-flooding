...
This paper is a first step in understanding the impact of Interest flooding attacks in NDN and exploring the solution space. While designing our mitigation algorithms, we exploit two key features of NDN architecture, namely routers maintaining state about the Interests they have forwarded and Data traffic taking the reverse path of the Interest traffic. We test the efficacy of our algorithms by simulating them on both a binary-tree topology designed to test the worst-case effectiveness of our algorithms as well as on a real ISP topology designed to provide insights our algorithms' behavior when deployed on a real network. While our results are promising and show great potential in mitigating Interest flooding DDoS attacks that can cause grave service degradation for legitimate users, we discuss some of the limitations of our evaluations and directions for future research. 

The attacker model that we used for our evaluations is simple and static - attackers send junk Interests as fast as possible. In future work, we plan to explore the impact of models where the attackers are more sophisticated and dynamically adapt their behavior and junk Interest sending patterns based on the network reaction. Further, in our evaluations, we assumed Interests are not satisfied by an intermediate router's cache and forwarded all the way to the producer.  We also ignored NDN's strategy layer that routes around failure and congestion in the network. In future work, we plan to study the impact of turning on these features. Finally, we also plan to perform extensive simulations on other real Internet topologies and realistic traffic patterns from legitimate users.