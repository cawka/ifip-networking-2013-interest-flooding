...
In Fig.~\ref{fig:large-scale}, we summarize our results averaged over 10 simulations runs for each mitigation algorithm. As in the case of the binary-tree topology, token bucket with per interface fairness is the most ineffective algorithm and satisfaction-based pushback algorithm is the most effective.   Interest satisfaction percentages for legitimate users are approximately 25\% and almost 100\% respectively for these two strategies.
