...
The Named Data Networking is a relatively new communication paradigm, but it already got a limited attention in the security community.
Lauinger~\cite{Lauinger:2010:Security--scalability} showed several attack vectors that can be applied to NDN. However, the work is specifically limited to a particular design choices implemented in CCNx software (\url{http://www.ccnx.org/}), rather than NDN architecture itself.  
...
While this work attempts to generalize findings for all ICNs, it largely does not go beyond the current design choices of CCNx.
...
In this paper we follow some suggestions by Gasti et al.~\cite{gasti2012ddos} and explore intrinsic potential of NDN architecture to sustain DDoS attacks, using the Interest flooding attack as a first step in this direction.
Our design for the attack mitigation relies in part on limiting number of outstanding Interests.  
Similar techniques were explored by Yi et al.~\cite{adaptive-forwarding,adaptive-tr} and Rozhnova and Fdida~\cite{rozhnova2012effective}, but applied only to facilitate performance in the NDN network.

The general area of denial of service attacks and their mitigation methods is one of the hottest topics of the past decade.
% Among all the body of work, 
The most related area to this paper is solutions to network-level flooding attacks, which include DDoS detection techniques~\cite{Chen:2007:Collaborative-detection, Jun:2011:DDoS-flooding}, methods to pushback of malicious traffic to the edged using various filtering techniques~\cite{Pushback, Tupakula:2003:A-practical-method, Argyraki:2005:Active-internet, Oikonomou:2006:A-framework-for-a-collaborative, Liu:2008:To-filter-or-to-authorize:, Chou:2009:Proactive-surge, Liu:2010:NetFence:-preventing}, various ticketing systems with conditional admission of traffic to the network~\cite{Yaar:2004:SIFF:-A-stateless, Yang:2005:A-DoS-limiting-network, Wendlandt:2006:Fastpass:-Providing, Natu:2007:Fine-grained-capabilities, Portcullis, Capabilities}, overlay-based filtering~\cite{Stone:2000:CenterTrack:-An-IP-overlay, Keromytis:2004:SOS:-An-architecture, Kline:2011:Shield:-DoS-filtering}, systems that attempt to use approximate IP traffic symmetry to estimate and filter out malicious traffic~\cite{Wang:2002:Detecting-SYN-flooding, Kreibich:2005:Using-packet, Mahimkar:2007:dFence:-Transparent}.
